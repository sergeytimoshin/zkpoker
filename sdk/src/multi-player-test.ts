...
import { scalarToBigint, type Scalar as GrumpkinScalar } from './grumpkin.js';
...
async function runMultiPlayerTest(numPlayers: number, outputDir: string) {
  console.log(`\n${'╔' + '═'.repeat(58) + '╗'}`);
  ...
  const maskHistory: { player: Player; inputCard: Card; outputCard: Card; nonce: GrumpkinScalar }[] = [];
...
`;
  ...
  const unmaskHistory: { player: Player; inputCard: Card; outputCard: Card }[] = [];
...
async function main() {
  ...
  const results: { players: number; success: boolean }[] = [];
