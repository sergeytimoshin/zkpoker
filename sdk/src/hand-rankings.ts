...
export interface HandRanking {
  primeProduct: bigint;  // Product of rank primes for the 5 cards
  rank: number;          // 0 = best (royal flush), 7461 = worst
  category: HandCategory;
  description: string;
  isFlush: boolean;      // Whether this is a flush variant
}
...
export function generateHandRankings(): {
  ...
  const allFlushCombos: { ranks: number[]; product: bigint }[] = [];
...
function hashLeaf(primeProduct: bigint, rank: bigint, isFlush: boolean): bigint {
...
export function buildMerkleTree(hands: HandRanking[]): {
  root: bigint;
  leaves: bigint[];
  tree: bigint[][];
} {
...
export function exportLookupTable(hands: HandRanking[]): string {
