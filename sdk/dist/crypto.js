...
// Point at infinity
const INFINITY = { x: 0n, y: 0n, isInfinite: true };
...
function mod(a, m = FIELD_ORDER) {
    return ((a % m) + m) % m;
}
function modInverse(a, m = FIELD_ORDER) {
...
export function addPlayerToCardMask(card, playerSecret) {
    ...
    const newPk = isFirstPlayer
        ...
        : pointAdd({ x: card.pk.x, y: card.pk.y, isInfinite: card.pk.isInfinite }, playerPub);
    ...
    if (!isFirstPlayer && !card.epk.isInfinite) {
        ...
        const msgPoint = pointAdd({ x: card.msg.x, y: card.msg.y, isInfinite: false }, epkScaled);
...
export function mask(card, nonce) {
    ...
    const newEpk = card.epk.isInfinite
        ...
        : pointAdd({ x: card.epk.x, y: card.epk.y, isInfinite: card.epk.isInfinite }, ephemeralPub);
    ...
    const newMsg = pointAdd({ x: card.msg.x, y: card.msg.y, isInfinite: false }, sharedSecret);
    return {
...
export function partialUnmask(card, playerSecret) {
    if (card.pk.isInfinite) {
    ...
    const newMsg = pointSub({ x: card.msg.x, y: card.msg.y, isInfinite: false }, decryptionShare);
    // pk_new = pk - player_pub
    const playerPub = scalarMul(playerSecret, GENERATOR);
    const newPk = pointSub({ x: card.pk.x, y: card.pk.y, isInfinite: card.pk.isInfinite }, playerPub);
    return {
        epk: card.epk,
        msg: { x: newMsg.x, y: newMsg.y },
        pk: { x: newPk.x, y: newPk.y, isInfinite: newPk.isInfinite },
...
export function addPlayerAndMask(card, playerSecret, nonce) {
