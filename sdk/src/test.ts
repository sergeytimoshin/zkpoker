...
// Simple EC point type for testing
interface ECPoint {
  x: bigint;
  y: bigint;
  isInfinite: boolean;
...
function modEC(a: bigint, m: bigint = FIELD_ORDER_EC): bigint {
  return ((a % m) + m) % m;
}

function modInverseEC(a: bigint, m: bigint = FIELD_ORDER_EC): bigint {
...
function pointAddSimple(p1: ECPoint, p2: ECPoint): ECPoint {
  if (p1.isInfinite) return p2;
  if (p2.isInfinite) return p1;
  if (p1.x === p2.x && modEC(p1.y + p2.y) === 0n) return { x: 0n, y: 0n, isInfinite: true };
...
function scalarMulSimple(scalar: bigint, point: ECPoint): ECPoint {
  if (point.isInfinite || scalar === 0n) return { x: 0n, y: 0n, isInfinite: true };
  let result: ECPoint = { x: 0n, y: 0n, isInfinite: true };
  let current = point;
  let s = modEC(scalar);
  while (s > 0n) {
    if (s & 1n) result = pointAddSimple(result, current);
...
import { cardCommitment } from './game-state.js';
