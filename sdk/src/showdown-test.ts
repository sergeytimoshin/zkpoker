...
interface ShowdownTestCase {
  name: string;
  // Game state before
  stackP1Before: number;
  stackP2Before: number;
  pot: number;
  dealer: number;
  // Hand ranks
  p1Ranks: number[];  // 5 rank indices for P1's best hand
  p1IsFlush: boolean;
  p2Ranks: number[];  // 5 rank indices for P2's best hand
  p2IsFlush: boolean;
  // Expected outcome
  expectedWinner: 'p1' | 'p2' | 'tie';
}
...
function generateShowdownToml(
  testCase: ShowdownTestCase,
  ...
  const {
    stackP1Before, stackP2Before, pot, dealer,
    p1Ranks, p1IsFlush, p2Ranks, p2IsFlush
  } = testCase;
  ...
  let winner: number;
  let finalStackP1: number;
  let finalStackP2: number;
...
async function main() {
  ...
  const testCases: ShowdownTestCase[] = [
    {
