 ...
 */
import { mod, addPoints, scalarMul, pointAtInfinity, scalarFromBigint, } from "./grumpkin.js";
...
export const PEDERSEN_GENERATORS = [
    {
...
export const PEDERSEN_LENGTH_GENERATOR = {
 ...
 */
export function pedersenHash(inputs) {
    if (inputs.length > PEDERSEN_GENERATORS.length) {
        throw new Error(`Pedersen hash supports up to ${PEDERSEN_GENERATORS.length} inputs, got ${inputs.length}`);
    }
 ...
 */
export function pedersenCommit(inputs) {
    if (inputs.length > PEDERSEN_GENERATORS.length) {
        throw new Error(`Pedersen commit supports up to ${PEDERSEN_GENERATORS.length} inputs, got ${inputs.length}`);
    }
    let result = pointAtInfinity();
    for (let i = 0; i < inputs.length; i++) {
        const scalar = scalarFromBigint(mod(inputs[i]));
